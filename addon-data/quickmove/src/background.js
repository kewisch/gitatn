...
async function spinWith(func, ...args) {
  ...
  try {
    gSpinLevel++;
    browser.action.setIcon({ path: "/images/spinner.svg" });
    ...
    if (gSpinLevel == 0) {
      browser.action.setIcon({ path: "/images/addon.svg" });
...
async function processSelectedMessages(folder, operation = "move", goToFolder = false) {
  ...
  if (goToFolder) {
    await browser.mailTabs.update(tab.id, { displayedFolderId: folderId }).catch(() => {});
  }
...
browser.commands.onCommand.addListener(async (name) => {
  let popupOrder = [browser.messageDisplayAction, browser.action];
